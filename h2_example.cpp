...
int main(int argc, char* argv[]) {
...

  {
    ...
    convertHmat2Dense(c1, c2, d, a_rebuilt, a_rebuilt.LDA);
...

  {
    start("verify H2");
    Matrix a_ref(m, n, m), a_rebuilt(m, n, m);
    convertH2mat2Dense(c1, c2, bi, bj, d, a_rebuilt, a_rebuilt.LDA);
    P2Pnear(fun, &c1[0], &c2[0], dim, a_ref);
  ...
  start("H2-vec");
  h2mv_complete(c1, c2, bi, bj, d, &x[0], &b[0]);
  stop("H2-vec");

  std::vector<double> b_ref(m);

  mvec_kernel(fun, &c1[0], &c2[0], dim, &x[0], &b_ref[0]);
