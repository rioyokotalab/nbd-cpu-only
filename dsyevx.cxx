...
int main(int argc, char* argv[]) {
  ...
  double abs_tol = argc > 4 ? atof(argv[4]) : 1e-3;
  const int64_t k_begin = argc > 5 ? atol(argv[5]) : 1;
  const int64_t k_end = argc > 6 ? atol(argv[6]) : k_begin;
  bool print_csv_header = argc > 7 ? (atol(argv[7]) == 1) : true;
  ...
  double dsyevx_time = MPI_Wtime();

  compute_selected_eigenvalues(&A, k_begin, k_end, abs_tol, computed_ev.data());

  MPI_Barrier(MPI_COMM_WORLD);
  dsyevx_time = MPI_Wtime() - dsyevx_time;

  // Print outputs
  if (mpi_rank == 0) {
#ifndef OUTPUT_CSV
    printf("NThreads=%d N=%d Kernel=%s Geometry=%s\n"
           "K_Begin=%d K_End=%d Abs_Tol=%.1e DSYEVX_Time=%.5lf\n",
           omp_get_max_threads(), Nbody, kernel_name.c_str(), geom_name.c_str(),
           (int)k_begin, (int)k_end, abs_tol, dsyevx_time);
#else
    if (print_csv_header == 1) {
      printf("nthreads,N,kernel,geometry,k_begin,k_end,abs_tol,dsyevx_time\n");
    }
    printf("%d,%d,%s,%s,%d,%d,%.1e,%.5lf\n",
           omp_get_max_threads(), Nbody, kernel_name.c_str(), geom_name.c_str(),
           (int)k_begin, (int)k_end, abs_tol, dsyevx_time);
#endif
  }
